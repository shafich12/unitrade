...
if(isset($_GET['order_id'])){
    ...
    $order = get_order_controller($order_id)[0];
    $order_details = get_order_details_controller($order_id);
    $total = 0;
}else{
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="success-text">
                        ...
                        <p class="font-weight-bold">Transaction ID: <?php echo $order['invoice_no'] ?></p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="product-order">
                        <?php if(count($order_details) > 0){ 
                            foreach($order_details as $item){
                                $pduct = select_one_product_controller($item['product_id']);
                                $total += $pduct['product_price'] * $item['qty']; 
                        ?>

                        <div class="row product-order-detail">
                            <div class="col-3"><img src="../<?php echo $pduct['product_image'] ?>" alt=""
                                    class="img-fluid blur-up lazyload"></div>
                            <div class="col-3 order_detail">
                                <div>
                                    <h4>product name</h4>
                                    <h5><?php echo $pduct['product_title'] ?></h5>
                                </div>
                            </div>
                            <div class="col-3 order_detail">
                                <div>
                            ...
                            <div class="col-3 order_detail">
                                <div>
                                    <h4>price</h4>
                                    <h5>GHS<?php echo number_format($pduct['product_price'] * $item['qty'], 2, '.', '');?></h5>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="final-total">
                            <h3>total <span>GHS<?php echo number_format($total, 2, '.', ''); ?></span></h3>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
